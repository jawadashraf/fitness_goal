...
<x-app-layout :assets="$assets ?? []">
    <div>
        <?php $id = $id ?? null;?>
        @if(isset($id))
            {!! Form::model($data, [ 'route' => ['exercise.update', $id], 'method' => 'patch', 'enctype' => 'multipart/form-data' ]) !!}
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="form-group col-md-4">
                                {{ Form::label('bodypart_ids', __('message.bodypart').' <span class="text-danger">*</span>',[ 'class' => 'form-control-label' ], false) }}
                                {{ Form::select('bodypart_ids[]', $selected_bodypart ?? [], old('bodypart_ids'), [
                            ...
                            <div class="form-group col-md-4">
                                ...
                                {{ Form::select('equipment_id', isset($id) ? [ optional($data->equipment)->id => optional($data->equipment)->title ] : [], old('equipment_id'), [
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-check">
                                            <div class="custom-control custom-radio d-inline-block col-4">
                                                ...
                                                {{ Form::radio('based', 'reps', old('based') || true, [ 'class' => 'form-check-input', 'id' => 'based-reps']) }}
                                            </div>
                                            <div class="custom-control custom-radio d-inline-block col-4">
                                                <label class="form-check-label" for="based-time"> {{__('message.time')}}(s)</label>
                                                {{ Form::radio('based', 'time', old('based'), [ 'class' => 'form-check-input', 'id' => 'based-time']) }}
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        ...
                                        <table id="table_list" class="table table-responsive">
                                            ...
                                            <tbody>
                                                ...
                                                <tr id="row_{{  $key }}" row="{{  $key }}" data-id="{{  $key }}">
                                                    <td>
                                                        <div class="form-group">
                                                            {{ Form::number('reps[]', $field['reps'] ?? null,[ 'placeholder' => __('message.reps'), 'class' =>'form-control', 'min' => 0 ]) }}
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="form-group">
                                                            {{ Form::number('time[]', $field['time'] ?? null,[ 'placeholder' => __('message.time'),'class' =>'form-control', 'min' => 0 ]) }}
                                                        </div>
                                                    </td>
                                                    <td class="weight">
                                                        <div class="form-group">
                                                            {{ Form::number('weight[]', $field['weight'] ?? null,[ 'placeholder' => __('message.weight'), 'class' =>'form-control', 'min' => 0 ]) }}
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="form-group">
                                                            {{ Form::number('rest[]', $field['rest'] ?? null,[ 'placeholder' => __('message.rest'), 'class' =>'form-control', 'min' => 0 ]) }}
                                                        </div>
                                                    </td>
                                                    <td>
                                                ...
                                                <tr id="row_0" row="0" data-id="0">
                                                    <td>
                                                        <div class="form-group">
                                                            {{ Form::number('reps[]', old('reps'),[ 'placeholder' => __('message.reps'), 'class' =>'form-control', 'min' => 0 ]) }}
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="form-group">
                                                            {{ Form::number('time[]', old('time'),[ 'placeholder' => __('message.time'),'class' =>'form-control', 'min' => 0 ]) }}
                                                        </div>
                                                    </td>
                                                    <td class="weight">
                                                        <div class="form-group">
                                                            {{ Form::number('weight[]', old('weight'),[ 'placeholder' => __('message.weight'), 'class' =>'form-control', 'min' => 0 ]) }}
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="form-group">
                                                            {{ Form::number('rest[]', old('rest'),[ 'placeholder' => __('message.rest'), 'class' =>'form-control', 'min' => 0 ]) }}
                                                        </div>
                                                    </td>
                            ...
                            <div id="exercise-duration" class="tab-pane fade {{ $type == 'duration' ? 'active show' : '' }}" role="tabpanel">
                                <div class="row duration_row">
                                    ...
                                    <div class="form-group col-md-2">
                                        ...
                                        {{ Form::label('hours',__('message.hours').'  <span class="text-danger">*</span>',['class'=>'form-control-label'],false) }}
                                        {{ Form::select('hours', isset($duration) ? [ $duration[0] => $duration[0] ] : [],old('hours'), [ 'class' => 'form-control select2js',
                                            'data-placeholder' => __('message.select_name',[ 'select' => __('message.hours') ]),
                                    ...
                                    <div class="form-group col-md-2">
                                        ...
                                        {{ Form::select('minute', isset($duration) ? [$duration[1] => $duration[1] ] : [], old('minute'), [ 'class' => 'form-control select2js',
                                            'data-placeholder' => __('message.select_name',[ 'select' => __('message.minute') ]),
                                            'data-ajax--url' => route('ajax-list', ['type' => 'minute']),
                                            ]) }}
                                    </div>
                                    <div class="form-group col-md-2">
                                        {{ Form::label('second',__('message.second').' <span class="text-danger">*</span>',['class'=>'form-control-label'],false) }}
                                        {{ Form::select('second', isset($duration) ? [$duration[2] => $duration[2] ] : [], old('second'), [ 'class' => 'form-control select2js',
                                            'data-placeholder' => __('message.select_name',[ 'select' => __('message.second') ]),
                                            'data-ajax--url' => route('ajax-list', ['type' => 'second']),
                                            ]) }}
                                    </div>
                                </div>
                            </div>
